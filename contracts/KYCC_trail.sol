...
contract KYC is Initializable, OwnableUpgradeable, UUPSUpgradeable {
    ...
    function addPerson(uint256 _id, address _wallet) internal {
        if (_id < 0) {
            revert ID_must_be_greater_than_zero();
        }
    ...
    function deletePerson(uint cid, uint _id) public {
        ...
        uint iid;
        uint uid;
        for (uint index = 0; index < nationalIDs.length; index++) {
            if (nationalIDs[index] == _id) iid = index;
        }
        if (people[_id].role == Roles.Admin) {
            for (uint index = 0; index < users.length; index++) {
                if (users[index] == _id) uid = index;
            }
    ...
    function getEducationI(
        uint256 id,
        uint i
    ) public view returns (Education memory) {
        return education[id][i];
    }

    function getExperiance(
        uint256 id
    ) public view returns (Experiance[] memory) {
        return experiance[id];
    }

    function getExperianceI(
        uint256 id,
        uint i
    ) public view returns (Experiance memory) {
